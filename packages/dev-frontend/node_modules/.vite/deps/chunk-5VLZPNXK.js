...
var import_dist4, import_dist5, import_dist6, __awaiter, logger, ForkEvent, Provider;
var init_lib9 = __esm({
  "../../node_modules/@ethersproject/abstract-provider/lib.esm/index.js"() {
    "use strict";
...
var init_version2 = __esm({
  "../../node_modules/@ethersproject/abstract-signer/lib.esm/_version.js"() {
    ...
    version2 = "abstract-signer/5.7.0";
  }
});
...
var import_dist10, import_dist11, import_dist12, __awaiter2, logger2, allowedTransactionKeys, forwardErrors, Signer, VoidSigner;
var init_lib10 = __esm({
  "../../node_modules/@ethersproject/abstract-signer/lib.esm/index.js"() {
    ...
    Signer = class _Signer {
      ...
      constructor() {
        logger2.checkAbstract(new.target, _Signer);
      ...
      checkTransaction(transaction) {
        for (const key2 in transaction) {
          if (allowedTransactionKeys.indexOf(key2) === -1) {
            logger2.throwArgumentError("invalid transaction key: " + key2, "transaction", transaction);
          }
        }
...
var import_dist13, import_dist14, import_dist15, import_bn, import_hash, minimalisticAssert, utils_1, utils_1$1, getNAF, getJSF, assert$1, base, inherits_browser, assert$2, short_1, curve_1, curves_1, hmacDrbg, assert$3, key, assert$4, signature, rand, assert$5, ec, elliptic_1, EC$1;
var init_elliptic = __esm({
  "../../node_modules/@ethersproject/signing-key/lib.esm/elliptic.js"() {
    ...
    ShortCurve.prototype._getEndomorphism = function _getEndomorphism(conf) {
      ...
      var lambda;
      if (conf.beta) {
        beta = new import_bn.default(conf.beta, 16).toRed(this.red);
      } else {
      ...
      if (conf.lambda) {
        lambda = new import_bn.default(conf.lambda, 16);
      } else {
    ...
    KeyPair.prototype._importPrivate = function _importPrivate(key2, enc) {
      this.priv = new import_bn.default(key2, enc || 16);
    ...
    EC.prototype.sign = function sign2(msg, key2, enc, options) {
      ...
      msg = this._truncateToN(new import_bn.default(msg, 16));
      var bytes = this.n.byteLength();
...
var import_dist19, import_dist20, import_dist21, logger3, _curve, SigningKey;
...
function formatNumber(value, name) {
  ...
  if (result.length > 32) {
    logger4.throwArgumentError("invalid length for " + name, "transaction:" + name, value);
  }
  return result;
}
  ...
  return {
    ...
    storageKeys: (storageKeys || []).map((storageKey, index) => {
      if (hexDataLength(storageKey) !== 32) {
        logger4.throwArgumentError("invalid access list storageKey", `accessList[${addr}:${index}]`, storageKey);
      }
  ...
  if (Array.isArray(value)) {
    return value.map((set, index) => {
      if (Array.isArray(set)) {
        if (set.length > 2) {
          logger4.throwArgumentError("access list expected to be [ address, storageKeys[] ]", `value[${index}]`, set);
        }
...
function _serializeEip1559(transaction, signature2) {
  if (transaction.gasPrice != null) {
    ...
    if (!gasPrice.eq(maxFeePerGas)) {
      logger4.throwArgumentError("mismatch EIP-1559 gasPrice != maxFeePerGas", "tx", {
        gasPrice,
        maxFeePerGas
      });
    }
...
function _serialize(transaction, signature2) {
  ...
  transactionFields.forEach(function(fieldInfo) {
    ...
    if (fieldInfo.length && value.length !== fieldInfo.length && value.length > 0) {
      logger4.throwArgumentError("invalid length for " + fieldInfo.name, "transaction:" + fieldInfo.name, value);
    }
  ...
  if (transaction.chainId != null) {
    ...
    if (typeof chainId !== "number") {
      logger4.throwArgumentError("invalid transaction.chainId", "transaction", transaction);
    }
  ...
  if (chainId !== 0) {
    ...
    v += chainId * 2 + 8;
    if (sig.v > 28 && sig.v !== v) {
      logger4.throwArgumentError("transaction.chainId/signature.v mismatch", "signature", signature2);
    }
  } else if (sig.v !== v) {
    logger4.throwArgumentError("transaction.chainId/signature.v mismatch", "signature", signature2);
  }
...
function serialize(transaction, signature2) {
  if (transaction.type == null || transaction.type === 0) {
    if (transaction.accessList != null) {
      logger4.throwArgumentError("untyped transactions do not support accessList; include type: 1", "transaction", transaction);
    }
  ...
  return logger4.throwError(`unsupported transaction type: ${transaction.type}`, Logger.errors.UNSUPPORTED_OPERATION, {
...
function _parseEipSignature(tx, fields, serialize2) {
  try {
    ...
    logger4.throwArgumentError("invalid v for transaction type: 1", "v", fields[0]);
  }
...
function _parseEip1559(payload) {
  ...
  if (transaction.length !== 9 && transaction.length !== 12) {
    logger4.throwArgumentError("invalid component count for transaction type: 2", "payload", hexlify(payload));
  }
...
function _parseEip2930(payload) {
  ...
  if (transaction.length !== 8 && transaction.length !== 11) {
    logger4.throwArgumentError("invalid component count for transaction type: 1", "payload", hexlify(payload));
  }
  const tx = {
    type: 1,
    chainId: handleNumber(transaction[0]).toNumber(),
...
function _parse(rawTransaction) {
  ...
  if (transaction.length !== 9 && transaction.length !== 6) {
    logger4.throwArgumentError("invalid raw transaction", "rawTransaction", rawTransaction);
  }
  const tx = {
...
function parse(rawTransaction) {
  ...
  return logger4.throwError(`unsupported transaction type: ${payload[0]}`, Logger.errors.UNSUPPORTED_OPERATION, {
    operation: "parseTransaction",
    transactionType: payload[0]
  });
}
var import_dist25, import_dist26, import_dist27, logger4, TransactionTypes, transactionFields, allowedTransactionKeys2;
...
export {
  ForkEvent,
  Provider,
  init_lib9 as init_lib,
  Signer,
  VoidSigner,
  lib_exports,
  init_lib10 as init_lib2,
  require_hash,
  SigningKey,
  recoverPublicKey,
  computePublicKey,
  init_lib11 as init_lib3,
  TransactionTypes,
  computeAddress,
  recoverAddress,
  accessListify,
  serialize,
  parse,
  init_lib12 as init_lib4
};
//# sourceMappingURL=chunk-5VLZPNXK.js.map
